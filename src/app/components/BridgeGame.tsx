...
// ─── Types ───────────────────────────────────────────────────────────────────
interface Plat { id: string; x: number; w: number; y: number; label: string; isStart?: boolean; isGoal?: boolean; isDeadEnd?: boolean; }
interface Gap {
  fromId: string; toId: string; a: number; b: number; c: number; tol: number;
  isDeadEnd?: boolean;      // L2: no matching material exists for this gap
  inverseMode?: boolean;    // L3: 'a' hidden, show macro triangle
  macroHyp?: number;        // L3: reference hypotenuse length (meters)
  macroBLeg?: number;       // L3: reference b-leg (meters)
  macroPlatRefId?: string;  // L3: platform at far end of hypotenuse
}
...
// ─── Level 2 data ─────────────────────────────────────────────────────────────
// RESOURCE MANAGEMENT TRAP — 6 platforms, 2 choice points:
//   Choice 1 (P1): go UP to P2 (uses 17m prematurely) or DOWN to P3 (correct, uses 10m)
//   Both P2 and P3 lead to junction P4 (via P2→P4 or P3→P4)
//   Choice 2 (P4): go UP to P5 (correct step 3) or shortcut direct to P6/GOAL (c=12, not in inventory)
//   Final: P5→P6 needs 17m — if wasted on P1→P2, player is stuck here!
// Inventory: [5m×1, 10m×1, 13m×1, 17m×1] — matches ONLY the correct path P1→P3→P4→P5→P6
const L2_PLATS: Plat[] = [
  { id: 'p1', x: 28,  w: 80, y: 248, label: 'START', isStart: true },
  { id: 'p2', x: 218, w: 80, y: 65,  label: 'P-2'                  }, // upper — reachable (17m exists) but wastes it
  { id: 'p3', x: 165, w: 78, y: 365, label: 'P-3'                  }, // lower correct
  { id: 'p4', x: 360, w: 80, y: 248, label: 'P-4'                  }, // junction
  { id: 'p5', x: 502, w: 78, y: 70,  label: 'P-5'                  }, // upper correct
  { id: 'p6', x: 628, w: 80, y: 248, label: 'GOAL', isGoal: true   },
];
const L2_GAPS: Gap[] = [
  // gap 0: P1→P2 — 17m bridge exists! But using it here leaves no 17m for the final P5→P6 step
  { fromId: 'p1', toId: 'p2', a: 8, b: 15, c: 17, tol: 0.3 },
  // gap 1: P2→P4 — auto-advance from P2; also uses 10m, which then blocks correct P4→P5 path
  { fromId: 'p2', toId: 'p4', a: 6, b: 8,  c: 10, tol: 0.3 },
  // gap 2: P1→P3 — correct step 1 (saves the 17m for the end)
  { fromId: 'p1', toId: 'p3', a: 6, b: 8,  c: 10, tol: 0.3 },
  // gap 3: P3→P4 — correct step 2, forced single path from P3
  { fromId: 'p3', toId: 'p4', a: 3, b: 4,  c: 5,  tol: 0.3 },
  // gap 4: P4→P5 — correct step 3
  { fromId: 'p4', toId: 'p5', a: 5, b: 12, c: 13, tol: 0.3 },
  // gap 5: P4→P6 direct shortcut — c=12 never in inventory → natural block, go back
  { fromId: 'p4', toId: 'p6', a: 12, b: 0, c: 12, tol: 0.3 },
  // gap 6: P5→P6 — correct step 4, final; needs 17m (wasted if used on P1→P2)
  { fromId: 'p5', toId: 'p6', a: 8, b: 15, c: 17, tol: 0.3 },
];
const L2_VALID = [2, 3, 4, 6]; // indices of correct path gaps (4 progress dots)
const L2_INV: Inv[] = [{ length: 5, count: 1 }, { length: 10, count: 1 }, { length: 13, count: 1 }, { length: 17, count: 1 }];

// ─── Level 3 data ─────────────────────────────────────────────────────────────
// CONSECUTIVE FORKS + DECOY MATERIALS + HIDDEN DATA — 7 platforms:
//   Fork 1 (P1): UP to P2 (vertical b=9, correct) or RIGHT to P3 (horizontal, structural dead-end)
//   Forced: P2→P4 (a=3,b=4,c=5 diagonal, only exit from P2)
//   Fork 2 (P4): RIGHT to P5 (horizontal a=12 HIDDEN, inverseMode, correct)
//                or DOWN to P6 (vertical b=8, structural dead-end)
//   Final: P5→P7 GOAL (a=5,b=12,c=13)
// Inventory: [9m,5m,12m,13m] correct + [8m,15m] decoys (8m→P4→P6 dead-end; 15m fails all correct)
// Macro triangle for P4→P5: P1→P5 shows c=15,b=9 → player deduces a=12
const L3_PLATS: Plat[] = [
  { id: 'p1', x: 28,  w: 82, y: 315, label: 'START', isStart: true   },
  { id: 'p2', x: 110, w: 80, y: 180, label: 'P-2'                    }, // correct: vertical 9m up
  { id: 'p3', x: 190, w: 75, y: 315, label: 'P-3', isDeadEnd: true   }, // trap: horizontal dead-end from P1
  { id: 'p4', x: 285, w: 80, y: 235, label: 'P-4'                    }, // junction (only exit from P2)
  { id: 'p5', x: 420, w: 80, y: 235, label: 'P-5'                    }, // correct: hidden horizontal 12m
  { id: 'p6', x: 380, w: 75, y: 315, label: 'P-6', isDeadEnd: true   }, // trap: vertical 8m dead-end
  { id: 'p7', x: 568, w: 82, y: 330, label: 'GOAL', isGoal: true     },
];
const L3_GAPS: Gap[] = [
  // gap 0: P1→P2 — correct step 1: pure vertical 9m up
  { fromId: 'p1', toId: 'p2', a: 0,  b: 9,  c: 9,  tol: 0.3 },
  // gap 1: P1→P3 — trap: horizontal 12m to structural dead-end
  { fromId: 'p1', toId: 'p3', a: 12, b: 0,  c: 12, tol: 0.3 },
  // gap 2: P2→P4 — correct step 2: forced (only exit from P2)
  { fromId: 'p2', toId: 'p4', a: 3,  b: 4,  c: 5,  tol: 0.3 },
  // gap 3: P4→P5 — correct step 3: hidden horizontal; macro triangle P1→P5 shows c=15,b=9 → a=12
  { fromId: 'p4', toId: 'p5', a: 12, b: 0,  c: 12, tol: 0.3,
    inverseMode: true, macroHyp: 15, macroBLeg: 9, macroPlatRefId: 'p1' },
  // gap 4: P4→P6 — trap: vertical 8m to structural dead-end (8m decoy enables this)
  { fromId: 'p4', toId: 'p6', a: 0,  b: 8,  c: 8,  tol: 0.3 },
  // gap 5: P5→P7 — correct step 4: final bridge
  { fromId: 'p5', toId: 'p7', a: 5,  b: 12, c: 13, tol: 0.3 },
];
const L3_VALID = [0, 2, 3, 5]; // correct path gap indices (4 progress dots)
const L3_INV: Inv[] = [
  { length: 9,  count: 1 },  // P1→P2 correct
  { length: 5,  count: 1 },  // P2→P4 correct
  { length: 12, count: 1 },  // P4→P5 correct (hidden — player must deduce from macro triangle)
  { length: 13, count: 1 },  // P5→P7 correct
  { length: 8,  count: 1 },  // decoy: enables P4→P6 dead-end; fails all correct bridges
  { length: 15, count: 1 },  // decoy: fails all correct bridges (closest c=13, diff=2 > tol=0.3)
];

interface LevelData { plats: Plat[]; gaps: Gap[]; validGaps: number[]; initInv: Inv[]; goalId: string; }
const LEVELS: Record<number, LevelData> = {
  2: { plats: L2_PLATS, gaps: L2_GAPS, validGaps: L2_VALID, initInv: L2_INV, goalId: 'p6' },
  3: { plats: L3_PLATS, gaps: L3_GAPS, validGaps: L3_VALID, initInv: L3_INV, goalId: 'p7' },
};

function getP(plats: Plat[], id: string) { return plats.find(p => p.id === id)!; }
function anch(plats: Plat[], g: Gap) {
  const f = getP(plats, g.fromId), t = getP(plats, g.toId);
  return { la: { x: f.x + f.w, y: f.y }, ra: { x: t.x, y: t.y } };
}
...
function drawPlat(ctx: CanvasRenderingContext2D, p: Plat, t: number, targets: string[], charPlatId: string) {
  ...
  const edgeCol = p.isGoal ? '#00e676' : p.isDeadEnd ? '#f87171' : isCurrent ? '#4db3ff' : '#1e4a7a';
  ...
  ctx.globalAlpha = 1;
  ctx.fillStyle = p.isGoal ? '#071a0e' : p.isDeadEnd ? '#1a0808' : '#06111e';
  ...
  if (p.isDeadEnd) { ctx.fillText('⚠', x + w / 2, y + 10); ctx.fillText(p.label, x + w / 2, y + 26); }
  else ctx.fillText(p.label, x + w / 2, y + PLAT_H / 2 + 1);
  ctx.restore();
}
...
function drawBuiltBridge(ctx: CanvasRenderingContext2D, la: Pt, ra: Pt, c: number) {
  ctx.save(); beamLine(ctx, la, ra, '#2d5a1b', '#4a8a2a');
  chip(ctx, `${Number.isInteger(c) ? c : c.toFixed(0)}m ✓`, (la.x+ra.x)/2, (la.y+ra.y)/2 - 12, '#00e676');
  ctx.restore();
}
...
function drawExtBridge(ctx: CanvasRenderingContext2D, la: Pt, ra: Pt, instLen: number, correctC: number, prog: number) {
  ...
  const curPx = (instLen/correctC)*cpx * prog; if (curPx < 1) return;
...
function drawFailBridge(ctx: CanvasRenderingContext2D, la: Pt, ra: Pt, instLen: number, correctC: number, kind: FailKind) {
  const dx = ra.x-la.x, dy = ra.y-la.y;
  const cpx = Math.sqrt(dx*dx + dy*dy); if (cpx === 0) return;
  const ux = dx/cpx, uy = dy/cpx;
  ...
  ctx.save();
  if (kind === 'short') {
    const ex = la.x + ux*chosenPx, ey = la.y + uy*chosenPx + 40;
    ...
    for (let i = 0; i < 8; i++) {
      const ang = (i/8)*Math.PI*2; const sr = 12;
      ctx.globalAlpha = 0.6;
...
function drawDims(ctx: CanvasRenderingContext2D, g: Gap, la: Pt, ra: Pt, hideA = false) {
  ...
  if (g.a > 0 && !hideA) chip(ctx, `a=${g.a}`, (la.x + corner.x)/2, la.y + (vd > 0 ? -12 : 16), '#5da5ff');
  else if (hideA)         chip(ctx, 'a = ?', (la.x + corner.x)/2, la.y + (vd > 0 ? -12 : 16), '#f87171');
  if (g.b > 0) chip(ctx, `b=${g.b}`, corner.x + 20, (corner.y + ra.y)/2, '#50c878');
  ctx.restore();
}

function drawMacroTriangle(ctx: CanvasRenderingContext2D, plats: Plat[], g: Gap) {
  if (!g.inverseMode || !g.macroHyp || !g.macroBLeg || !g.macroPlatRefId) return;
  const refP = getP(plats, g.macroPlatRefId);
  const toP  = getP(plats, g.toId);
  // A = anchor at ref platform right edge (lower-left of macro triangle)
  const A = { x: refP.x + refP.w, y: refP.y };
  // C = anchor at destination platform left edge (upper-right)
  const C = { x: toP.x, y: toP.y };
  // B = right-angle corner (same x as A, same y as C)
  const B = { x: A.x, y: C.y };

  ctx.save();
  ctx.lineWidth = 2; ctx.setLineDash([9, 6]);
  // Hypotenuse (blue): A → C
  ctx.strokeStyle = 'rgba(93,165,255,0.5)';
  ctx.beginPath(); ctx.moveTo(A.x, A.y); ctx.lineTo(C.x, C.y); ctx.stroke();
  // Vertical leg (green, known b): A → B
  ctx.strokeStyle = 'rgba(80,200,120,0.6)';
  ctx.beginPath(); ctx.moveTo(A.x, A.y); ctx.lineTo(B.x, B.y); ctx.stroke();
  // Horizontal leg (red, hidden a): B → C
  ctx.strokeStyle = 'rgba(248,113,113,0.6)';
  ctx.beginPath(); ctx.moveTo(B.x, B.y); ctx.lineTo(C.x, C.y); ctx.stroke();
  ctx.setLineDash([]);

  // Right-angle square at B: direction adapts based on whether A is below or above B
  const sq = 9;
  const sqDir = A.y > B.y ? 1 : -1;
  ctx.strokeStyle = 'rgba(148,163,184,0.5)'; ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(B.x + sq, B.y);
  ctx.lineTo(B.x + sq, B.y + sqDir * sq);
  ctx.lineTo(B.x, B.y + sqDir * sq);
  ctx.stroke();

  // Labels
  chip(ctx, `c=${g.macroHyp}m`, (A.x + C.x) / 2 - 20, (A.y + C.y) / 2 - 10, '#5da5ff');
  chip(ctx, `b=${g.macroBLeg}m`, A.x - 32, (A.y + B.y) / 2, '#50c878');
  chip(ctx, 'a = ?', (B.x + C.x) / 2, B.y + sqDir * 16, '#f87171');
  ctx.restore();
}
...
// ─── Canvas render ────────────────────────────────────────────────────────────
function renderFrame(ctx: CanvasRenderingContext2D, gs: GS, t: number, walkFrame: number, lv: LevelData) {
  ctx.clearRect(0, 0, CW, CH);
  drawBg(ctx);
  drawWater(ctx, t);

  const gap = gs.gapIdx !== null ? lv.gaps[gs.gapIdx] : null;
  const reachable = gs.phase === 'pick_target'
    ? lv.gaps.map((g, i) => ({ g, i })).filter(({ g, i }) => g.fromId === gs.charPlatId && !gs.built.includes(i)).map(({ g }) => g.toId)
    : [];

  // Macro triangle overlay for L3 (draw before platforms so platforms render on top)
  if (gap?.inverseMode && (gs.phase === 'pick_material' || gs.phase === 'building' || gs.phase === 'fail')) {
    drawMacroTriangle(ctx, lv.plats, gap);
  }
  // Also show macro triangle when it's the active gap but not yet built
  if (!gap) {
    const invGap = lv.gaps.find((g, i) => g.inverseMode && !gs.built.includes(i));
    if (invGap && gs.phase === 'pick_target' && gs.charPlatId === invGap.fromId) drawMacroTriangle(ctx, lv.plats, invGap);
  }

  // Platforms
  for (const p of lv.plats) drawPlat(ctx, p, t, reachable, gs.charPlatId);

  // Built bridges
  for (const idx of gs.built) {
    const g = lv.gaps[idx];
    const { la, ra } = anch(lv.plats, g);
    drawBuiltBridge(ctx, la, ra, g.c);
  }

  // Active gap rendering
  if (gap) {
    const { la, ra } = anch(lv.plats, gap);
    const hideA = !!(gap.inverseMode);
    if (gs.phase === 'pick_material') {
      drawDims(ctx, gap, la, ra, hideA);
      ...
      drawDims(ctx, gap, la, ra, hideA);
      if (gs.installedLen !== null && gs.failKind) drawFailBridge(ctx, la, ra, gs.installedLen, gap.c, gs.failKind);
    } else if (gs.phase === 'pick_target') {
      for (const { g: rg } of lv.gaps.map((g2, i) => ({ g: g2, i })).filter(({ g: g2, i }) => g2.fromId === gs.charPlatId && !gs.built.includes(i))) {
        drawDims(ctx, rg, anch(lv.plats, rg).la, anch(lv.plats, rg).ra, !!(rg.inverseMode));
      }
    }
  } else if (gs.phase === 'pick_target') {
    for (const { g: rg } of lv.gaps.map((g2, i) => ({ g: g2, i })).filter(({ g: g2, i }) => g2.fromId === gs.charPlatId && !gs.built.includes(i))) {
  ...
  // Character
  const charPlat = getP(lv.plats, gs.charPlatId);
  let cx = charPlat.x + charPlat.w / 2, cy = charPlat.y;
  if (gs.phase === 'walking' && gap) {
    const fp = getP(lv.plats, gap.fromId), tp = getP(lv.plats, gap.toId);
    cx = (fp.x + fp.w - 12) + (tp.x + 12 - (fp.x + fp.w - 12)) * gs.walkProg;
    cy = fp.y + (tp.y - fp.y) * gs.walkProg;
  } else if (gs.phase === 'complete') {
    const goalP = getP(lv.plats, lv.goalId);
...
// ─── Main component ───────────────────────────────────────────────────────────
interface Props { levelId: number; onBack?: () => void; onComplete?: (wrong: number) => void; }

function mkGs(lv: LevelData): GS {
  return { phase: 'pick_target', charPlatId: lv.plats[0].id, gapIdx: null, selLen: null, installedLen: null, inv: lv.initInv.map(i => ({ ...i })), built: [], buildProg: 0, walkProg: 0, failKind: null, wrong: 0 };
}

export function BridgeGame({ levelId, onBack, onComplete }: Props) {
  const lv = LEVELS[levelId] ?? LEVELS[2];
  const [showIntro, setShowIntro] = useState(true);
  const canvasRef  = useRef<HTMLCanvasElement>(null);
  const mountedRef = useRef(true);
  const gsRef      = useRef<GS>(mkGs(lv));
  const animTRef   = useRef(0);
  const walkFrameRef = useRef(0);
  const onCompleteCalledRef = useRef(false);
  const [gs, setGs] = useState<GS>(() => mkGs(lv));
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      if (canvas) {
        ...
        if (ctx) renderFrame(ctx, gsRef.current, animTRef.current, walkFrameRef.current, lv);
      }
  ...
  useEffect(() => {
    ...
    const avail = lv.gaps.map((g, i) => ({ g, i })).filter(({ g, i }) => g.fromId === gs.charPlatId && !gs.built.includes(i));
  ...
  useEffect(() => {
    ...
    const run = (time: number) => {
      ...
      setGs(prev => {
        ...
        if (newCharPlatId === lv.goalId) {
          return { ...prev, phase: 'complete', charPlatId: newCharPlatId, built: newBuilt, walkProg: 1 };
        }
        const avail = lv.gaps.map((g2, i) => ({ g: g2, i })).filter(({ g: g2, i }) => g2.fromId === newCharPlatId && !newBuilt.includes(i));
        if (avail.length === 1) return { ...prev, phase: 'pick_material', charPlatId: newCharPlatId, built: newBuilt, gapIdx: avail[0].i, selLen: null, installedLen: null, walkProg: 1 };
  ...
  // ─── Handlers ─────────────────────────────────────────────────────
  const reachable = lv.gaps.map((g, i) => ({ g, i })).filter(({ g, i }) => g.fromId === gs.charPlatId && !gs.built.includes(i)).map(({ g }) => g.toId);
  ...
  const handleCanvasClick = (e: MouseEvent<HTMLCanvasElement>) => {
    ...
    for (const platId of reachable) {
      const p = getP(lv.plats, platId);
      if (mx >= p.x && mx <= p.x + p.w && my >= p.y && my <= p.y + PLAT_H) {
        const gapIdx = lv.gaps.findIndex(g => g.fromId === gs.charPlatId && g.toId === platId && !gs.built.includes(lv.gaps.indexOf(g)));
  ...
  const handleRetry = () => {
    ...
    if (avail.length === 1) setGs(prev => ({ ...prev, phase: 'pick_material', gapIdx: avail[0].i, installedLen: null, selLen: null }));
    else setGs(prev => ({ ...prev, phase: 'pick_target', gapIdx: null, installedLen: null, selLen: null }));
  };

  const handleRestart = () => { onCompleteCalledRef.current = false; setGs(mkGs(lv)); };
  ...
  const progressDots = lv.validGaps.map(i => gs.built.includes(i) ? 'done' : i === gs.gapIdx ? 'active' : 'pending');

  // Level name
  const levelName = levelId === 2 ? '路径分叉 · 二选一' : '连续迷宫 · 干扰材料';
  const levelNum  = String(levelId).padStart(2, '0');

  // Sidebar: show inverse hint for L3 inverse gaps
  const showInverseGuide = !!(gap?.inverseMode);

  if (showIntro) return <LevelIntro levelId={levelId} onStart={() => setShowIntro(false)} />;
    ...
    <div className="flex flex-col" style={{ minHeight: '100vh', background: '#020b18' }}>
      ...
      <div className="flex items-center justify-between px-4 py-2 shrink-0" style={{ background: 'rgba(0,0,0,0.5)', borderBottom: '1px solid rgba(30,80,180,0.25)' }}>
        <div className="flex items-center gap-3">
          {onBack && <button onClick={onBack} style={{ color: '#4db3ff', fontSize: '1.1rem', cursor: 'pointer', background: 'none', border: 'none', lineHeight: 1 }}>←</button>}
          <span style={{ color: '#4db3ff', fontFamily: 'monospace', fontSize: '0.78rem' }}>LEVEL {levelNum} · {levelName}</span>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-1.5">
            {progressDots.map((s, i) => (
              <div key={i} className="rounded-full" style={{ width: 8, height: 8, background: s === 'done' ? '#00e676' : s === 'active' ? '#ffc400' : '#0d2a4a' }} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-1 min-h-0 flex-col lg:flex-row">
        ...
        <div className="shrink-0 flex flex-col gap-3 p-3 overflow-y-auto" style={{ width: '100%', maxWidth: 300, minWidth: 252, background: 'rgba(0,0,0,0.38)', borderLeft: '1px solid rgba(20,80,180,0.2)' }}>
          ...
          {/* Inventory */}
          <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${lv.initInv.length <= 4 ? 2 : 3}, 1fr)` }}>
            {lv.initInv.map(({ length }) => {
              ...
              return (
                <button key={length} onClick={() => handleSelectMat(length)} disabled={disabled}
                  ...
                  style={{ cursor: disabled ? 'default' : 'pointer', opacity: disabled ? (count === 0 ? 0.28 : 0.45) : 1, background: isSel ? 'rgba(255,196,0,0.18)' : 'rgba(255,255,255,0.04)', border: `1.5px solid ${isSel ? '#ffc400' : 'rgba(20,80,180,0.35)'}`, boxShadow: isSel ? '0 0 12px rgba(255,196,0,0.25)' : undefined }}>
          ...
          {/* Inverse hint for L3 — shows setup only, no answer */}
          {showInverseGuide && gap && gs.phase !== 'complete' && (
            <div className="rounded-xl p-3" style={{ background: 'rgba(93,165,255,0.05)', border: '1px solid rgba(93,165,255,0.25)' }}>
              <p style={{ color: '#5da5ff', fontSize: '0.7rem', fontFamily: 'monospace', marginBottom: 6 }}>∑ 参考线索</p>
              <p style={{ color: '#93c5fd', fontFamily: 'monospace', fontSize: '0.76rem', lineHeight: 1.7 }}>大斜边 c = {gap.macroHyp}m</p>
              <p style={{ color: '#86efac', fontFamily: 'monospace', fontSize: '0.76rem', lineHeight: 1.7 }}>竖直 b = {gap.macroBLeg}m</p>
              <p style={{ color: '#334155', fontFamily: 'monospace', fontSize: '0.76rem', lineHeight: 1.7 }}>─────────────</p>
              <p style={{ color: '#94a3b8', fontFamily: 'monospace', fontSize: '0.76rem', lineHeight: 1.7 }}>已知 c 和 b，能求 a 吗？</p>
              <p style={{ color: '#fbbf24', fontFamily: 'monospace', fontSize: '0.76rem', lineHeight: 1.7 }}>a² = c² - b²</p>
              <p style={{ color: '#fb923c', fontFamily: 'monospace', fontSize: '0.76rem', lineHeight: 1.7 }}>a = ?</p>
            </div>
          )}

          {/* Standard gap info */}
          {gap && !showInverseGuide && gs.phase !== 'complete' && (
          ...
          {/* Inventory exhausted — all materials consumed, truly stuck */}
          {invEmpty && (
          ...
          {gs.selLen !== null && gs.phase === 'pick_material' && (
            <div className="rounded-xl p-2 text-center" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(20,80,180,0.2)' }}>
              <p style={{ color: '#94a3b8', fontSize: '0.73rem' }}>已选 <span style={{ color: '#ffc400', fontFamily: 'monospace' }}>{gs.selLen}m</span><span style={{ color: '#334155' }}> — 点击施工按钮</span></p>
            </div>
          )}

          {/* BUILD button */}
          {gs.phase === 'pick_material' && !invEmpty && (
            <button onClick={handleBuild} disabled={!canBuild}
              className="w-full rounded-xl py-3 text-white transition-all active:scale-95"
              style={{ cursor: canBuild ? 'pointer' : 'default', background: canBuild ? 'linear-gradient(135deg,#c2410c,#ea580c)' : 'rgba(255,255,255,0.04)', color: canBuild ? '#fff' : '#1e3a5f', border: canBuild ? 'none' : '1px solid rgba(20,80,180,0.2)', boxShadow: canBuild ? '0 4px 18px rgba(234,88,12,0.35)' : undefined, fontFamily: 'monospace', fontSize: '0.88rem', letterSpacing: 1 }}>
          ...
          {isFail && (
            <div className="rounded-xl p-3" style={{ background: gs.failKind === 'short' ? 'rgba(239,68,68,0.07)' : 'rgba(251,146,60,0.07)', border: `1px solid ${gs.failKind === 'short' ? 'rgba(239,68,68,0.3)' : 'rgba(251,146,60,0.3)'}` }}>
              ...
              <div className="flex gap-2 mt-2">
                <button onClick={handleRetry} className="flex-1 py-2 rounded-lg cursor-pointer transition-all active:scale-95" style={{ background: 'rgba(255,255,255,0.08)', color: '#e2e8f0', border: '1px solid rgba(255,255,255,0.1)', fontSize: '0.8rem' }}>撤回重选 ↩</button>
                <button onClick={handleRestart} className="flex-1 py-2 rounded-lg cursor-pointer" style={{ background: 'rgba(239,68,68,0.12)', color: '#f87171', border: '1px solid rgba(239,68,68,0.25)', fontSize: '0.8rem' }}>重置关卡</button>
              </div>
            </div>
          )}

          {/* Stuck: structural dead-end (no exits from this platform) */}
          {gs.phase === 'pick_target' && reachable.length === 0 && gs.charPlatId !== lv.goalId && (
            <div className="rounded-xl p-3" style={{ background: 'rgba(248,113,113,0.07)', border: '1px solid rgba(248,113,113,0.3)' }}>
              <p style={{ color: '#f87171', fontSize: '0.78rem', lineHeight: 1.6 }}>
                前方没有出路。<br />
                <span style={{ color: '#475569' }}>回头检查路线与材料分配。</span>
              </p>
              <button onClick={handleRestart} className="mt-2 w-full py-1.5 rounded-lg cursor-pointer text-sm"
          ...
          {gs.phase === 'pick_target' && reachable.length > 0 && (
            <div className="rounded-xl p-3" style={{ background: 'rgba(77,179,255,0.05)', border: '1px solid rgba(77,179,255,0.2)' }}>
              <p style={{ color: '#4db3ff', fontSize: '0.78rem', lineHeight: 1.6 }}>
                📍 选择目标平台<br />
                {levelId === 2 && <span style={{ color: '#475569' }}>仓库有限，路线二选一！</span>}
                {levelId === 3 && <span style={{ color: '#475569' }}>两个方向各有不同结局…</span>}
              </p>
            </div>
          )}
          ...
          {gs.phase === 'complete' && (
            <div className="rounded-xl p-4 flex flex-col gap-3" style={{ background: 'rgba(0,230,118,0.08)', border: '1px solid rgba(0,230,118,0.3)' }}>
              ...
              <div className="flex justify-around">
                <div className="text-center"><p style={{ color: '#334155', fontSize: '0.65rem' }}>错误次数</p><p style={{ color: gs.wrong === 0 ? '#00e676' : '#f87171', fontFamily: 'monospace', fontSize: '1.5rem' }}>{gs.wrong}</p></div>
                <div className="text-center"><p style={{ color: '#334155', fontSize: '0.65rem' }}>架桥数</p><p style={{ color: '#4db3ff', fontFamily: 'monospace', fontSize: '1.5rem' }}>{lv.validGaps.length}</p></div>
              </div>
              {gs.wrong === 0 && <p style={{ color: '#00e676', fontSize: '0.75rem', textAlign: 'center' }}>🏆 零失误！完美通关！</p>}
              <div className="flex gap-2">
                <button onClick={handleRestart} className="flex-1 py-2 rounded-lg cursor-pointer" style={{ background: 'rgba(255,255,255,0.06)', color: '#94a3b8', border: '1px solid rgba(255,255,255,0.1)', fontSize: '0.8rem' }}>重玩</button>
                {onBack && <button onClick={onBack} className="flex-1 py-2 rounded-lg cursor-pointer" style={{ background: 'rgba(0,230,118,0.2)', color: '#00e676', border: '1px solid rgba(0,230,118,0.35)', fontSize: '0.8rem' }}>关卡选择</button>}
              </div>
            </div>
          )}

          <div className="mt-auto pt-2" style={{ borderTop: '1px solid rgba(20,80,180,0.18)' }}>
            <p style={{ color: '#1e3a5f', fontSize: '0.68rem', fontFamily: 'monospace', textAlign: 'center' }}>观察规律 · 自主推导 · 匹配桥长</p>
          </div>
        </div>
      </div>
    </div>
